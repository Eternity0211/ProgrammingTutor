...
export default async function ProfilePage() {
  ...
  const generateSkillDataFromHistory = () => {
    ...
    feedbackHistory.forEach((log) => {
      const feedbackText =
        `${log.emotionFeedback} ${log.navigatorTips}`.toLowerCase();
          ...
          <Card className="md:col-span-2">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <Brain className="w-5 h-5 text-blue-500" /> C++ 能力画像
                (基于符号错误分析)
              </CardTitle>
                        ...
                        <div className="md:col-span-3 p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="flex gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-sm mt-1 italic leading-relaxed">
                                "{log.emotionFeedback}"
                              </p>
                            </div>
                          </div>
            ...
            <CardContent className="space-y-3 max-h-[400px] overflow-y-auto pr-1">
              {learningPath.length > 0 ? (
                learningPath.map((step: LearningPathStep) => (
                  <div
                    key={step.step}
                    className="p-3 border rounded-md border-l-4 border-l-blue-500"
                  >
                    <div className="flex justify-between items-center">
                      <p className="font-medium text-sm">
                        第 {step.step} 步：{step.topic}
                      </p>
