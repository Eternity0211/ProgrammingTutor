...
- 线性顺序 (Sequential)：变量的值沿一条直线更新，没有分支或循环。区间推导充满确定性，若结果越界则通常是 Must。
...
## 第三部分：理论依据
本项目的动态 Checker 体系并非凭空而建，而是建立在成熟的静态分析理论之上。理解这些理论可以帮助开发者更有信心地设计规则、解释误报并为未来扩展选定正确路径。

### 核心理论
1. **抽象解释（Abstract Interpretation）**  
   抽象解释是一种将程序的具体语义映射到“可计算抽象域”的框架。
   - 每个语言操作（加减、指针运算、条件判断等）都对应一个抽象转移函数，负责将输入抽象状态转换成输出抽象状态。
   - 在我们的 `engine` 中，这些函数主要定义在 `state.ts` 和 `env.ts`，并通过插件机制允许 Checker 扩展或覆盖。  
   - **保守性原则**是核心：抽象状态必须覆盖所有可能的具体运行情形，因此绝不会出现漏报。只有当抽象结果与合法集合完全不相交时才报告 Must-Issue。
2. **区间域（Interval Domain）**  
   区间是最简单、最高效的抽象域之一，当前引擎默认采用此模式。  
   - 每个变量用一个闭区间 `[l,u]` 表示可能值；特殊抽象元素包括 `⊥`（无效/不可能）和 `⊤`（任意值）。  
   - 区间运算支持所有基础算术、比较和位操作，并对整数溢出进行 Conservative 推演。  
   - 在处理循环或递归时，分析器执行固定点迭代，必要时通过 **widening**（上界提升）防止不收敛。这一过程会将逐渐扩大的区间最后提升到 `⊤`，对应之前所述的“发散”。

### 与控制流的结合
- 分析从抽象语法树构建 **控制流图（CFG）**，并转换为单静态赋值形式（SSA），便于数据流跟踪。
- **分支合并**使用区间的 **join** 运算，将不同路径的结果取并集。
- **循环回边**通过前述的 widening 处理，保障分析在有限时间内终止。

### 理论来源与验证
- 主要参考文献包括 Cousot & Cousot 的《Static Analysis by Abstract Interpretation》以及 Antoine Miné 关于区间分析的系列论文。
- Must/May 分类直接借鉴抽象解释中的安全域（safe] 和警告域（warning] 概念：前者严格保证覆盖实际行为，后者则标识可能性。

### 扩展前景
该体系具备良好的可扩展性——如果将来引入更精细的抽象域（如关系域、符号域、布尔域等），只需在 `engine` 中提供额外的抽象转移和合并逻辑，现有 Checker 仍可重用。新规则只需专注于具体“状态×行为”的判定逻辑，而无需重新发明分析结构。
以上内容为编写和扩展动态规则提供了权威的理论依据，确保每条Checker 都有清晰的数学保障。
---