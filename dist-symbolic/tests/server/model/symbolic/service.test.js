...
// Test Setup
// =============================================================================
const ERRORS_DIR = path_1.default.resolve(
  process.cwd(),
  "data/symbolic/ast-patterns/cpp/errors",
);
const WARNINGS_DIR = path_1.default.resolve(
  process.cwd(),
...
beforeAll(() => {
  if (
    !fs_1.default.existsSync(ERRORS_DIR) ||
    !fs_1.default.existsSync(WARNINGS_DIR)
  ) {
    console.warn(
      "⚠️ [Service Test] Pattern directories missing. Integration tests might return empty results.",
    );
  }
});
// =============================================================================
...
describe("Symbolic Analysis Service (Orchestrator)", () => {
  ...
  it("should aggregate both errors and warnings", async () => {
    ...
    `;
    const result = await (0, service_1.analyzeCode)(mixedCode);
    // 验证 Errors 模块工作正常
    // 注意：语法错误可能导致部分逻辑规则失效，但原生语法检测必须生效
    const hasSyntaxError = result.errors.some(
      (e) => e.ruleId === "CPP_SYNTAX_ERROR",
    );
    expect(hasSyntaxError).toBe(true);
    // 验证 Warnings 模块工作正常 (如果环境中有 CPP_NO_GOTO 规则)
    const rulePath = path_1.default.join(WARNINGS_DIR, "CPP_NO_GOTO.scm");
    if (fs_1.default.existsSync(rulePath)) {
      const hasGotoWarning = result.warnings.some(
        (e) => e.ruleId === "CPP_NO_GOTO",
      );
  ...
  it("should return enriched issue objects (not raw)", async () => {
    ...
    const result = await (0, service_1.analyzeCode)(code);
  ...
  it("should handle unexpected failures gracefully", async () => {
    ...
    const emptyCode = "";
    // 该操作不应抛出异常
    const result = await (0, service_1.analyzeCode)(emptyCode);
