...
import { buildCFG, CFGNode } from "../../../../../src/server/model/symbolic/dynamic/cfg";
...
function findBlock(nodes: CFGNode[], prefix: string): CFGNode {
  ...
  if (blocks.length === 0) throw new Error(`[Topology Error] Block prefix '${prefix}' not found.`);
  return blocks[0];
}
...
function findBlockWithNodeType(nodes: CFGNode[], nodeType: string): CFGNode {
  const block = nodes.find(n => n.statements.some(s => s.type === nodeType));
  if (!block) throw new Error(`[Topology Error] Block with AST node '${nodeType}' not found.`);
  return block;
}
...
describe("Dynamic Analysis - Advanced CFG Builder", () => {
  ...
  it("3. 应该穿透作用域，精准连接 For 循环中的 Break 和 Continue 目标块", async () => {
    ...
    const continueBlock = findBlockWithNodeType(cfg.nodes, "continue_statement");
